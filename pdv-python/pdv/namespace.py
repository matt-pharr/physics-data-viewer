...
- :class:`PDVNamespace`: a dict subclass used as the IPython user
  namespace. Blocks reassignment of ``pdv_tree``.
...
from pdv.errors import PDVProtectedNameError

# Names that must never be reassigned by user code.
_PROTECTED_NAMES: frozenset[str] = frozenset({"pdv_tree"})
...
class PDVNamespace(dict):
    ...
    assign to ``pdv_tree``. All other assignments behave normally.
...
def pdv_namespace(
    ...
    *,
    ...
    Excludes PDV internals (``pdv_tree`` and any name starting with
    ``_pdv``). Optionally filters private names (``_``-prefixed),
