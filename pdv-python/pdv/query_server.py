...
class QueryServer:
    ...
    def _handle_request(self, raw: bytes) -> dict[str, Any]:
        """Process one raw request payload and return the reply envelope.

        Pure with respect to sockets — parses *raw*, enforces the read-only
        whitelist, dispatches allowed queries while capturing the handler's
        reply via the thread-local response sink, and always returns a
        well-formed envelope (never raises). Extracted from :meth:`_run` so the
        routing logic can be exercised without a live ZeroMQ socket.

        Parameters
        ----------
        raw : bytes
            The JSON-encoded request payload received on the REP socket.

        Returns
        -------
        dict
            The reply envelope to send back. On a non-whitelisted type it
            carries ``query.not_allowed``; on a handler that produces no reply,
            ``query.no_response``; on any unexpected error, ``query.internal_error``.
        """
    ...
    def _run(self) -> None:
        ...
        while not self._shutdown.is_set():
            ...
            # _handle_request never raises, so a reply is always produced.
