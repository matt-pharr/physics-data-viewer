...
| Interrupt-safety heal | n/a (no threaded-region flag) | an IJulia preexecute hook clears a leaked `jl_in_threaded_region` flag (Base's `threading_run` lacks try/finally, so interrupting an `@threads` loop otherwise poisons every later `@threads :static` in the session) |
