...
describe("@slow Cross-boundary integration (Python + Electron)", { timeout: 180_000 }, () => {
  ...
  // Each describe below owns the tree/namespace state it needs (seeded in a
  // block-local beforeAll under a unique prefix) and reads no state from a
  // sibling block. `kernelId`, `router`, and `initialWorkingDir` are the only
  // shared values, and they are read-only until the trailing "kernel restart"
  // block. Ordering WITHIN a block is significant where a block models a
  // workflow (e.g. the context-menu sequence); ordering ACROSS blocks is not.
  ...
  describe("tree read queries", () => {
    ...
    describe("pdv.tree.get", () => {
      const key = "treeget_scalar";
      beforeAll(async () => {
        const seedResult = await km.execute(kernelId, { code: `pdv_tree['${key}'] = 42` });
        expect(seedResult.error).toBeUndefined();
      });

      it("mode=value -> Python returns value", async () => {
      ...
      it("mode=metadata -> Python returns metadata envelope", async () => {
        const response = await router.request(PDVMessageType.TREE_GET, {
          path: key,
          mode: "metadata",
        });
  ...
  describe("script registration and params", () => {
    ...
    // The two params tests deliberately share one registered script: the first
    // reads its params, the second edits the backing file and re-reads to prove
    // params reflect on-disk edits without re-registration. Registration is the
    // block's setup so neither test depends on the other having created it.
    describe("pdv.script.params (UUID-based, on-demand)", () => {
      ...
      beforeAll(async () => {
        ...
        solvePath = path.join(solveDir, "solve.py");
        await fs.writeFile(
          solvePath,
          [
      ...
      it("returns params on demand for UUID-based scripts", async () => {
      ...
      it("reflects file edits without re-registering", async () => {
        await fs.writeFile(
          solvePath,
          [
  ...
  describe("streaming output", () => {
    it("streaming iopub output arrives before execution completes", async () => {
      ...
      try {
        ...
        // Poll for the first chunk instead of a fixed sleep: it must arrive
        // while execution is still in flight (before executePromise resolves).
        await vi.waitFor(() => expect(streamTexts.length).toBeGreaterThan(0), {
          timeout: 5_000,
          interval: 20,
        });
  ...
  // A single workflow: `ctx_container` is created in beforeAll, then the tests
  // exercise create/rename/move/duplicate/delete/file-register against it in
  // order. This sequence is intentionally ordered; nothing outside this block
  // reads its state.
  describe("tree context-menu operations", () => {
    beforeAll(async () => {
      const changedPromise = waitForPush(router, PDVMessageType.TREE_CHANGED);
    ...
    it("pdv.tree.create_node creates an empty container and pushes tree.changed", async () => {
      ...
      expect(payload.path).toBe("ctx_probe");

      const push = await changedPromise;
      const pushPayload = push.payload as { changed_paths?: string[] };
      expect(pushPayload.changed_paths).toContain("ctx_probe");
      ...
      expect(nodes.find((n) => n.key === "ctx_probe")).toBeDefined();
    });
  ...
  // Trailing block: this is the only place `kernelId`/`router` are mutated, so
  // it must run last. afterAll shuts down whichever kernel id is current.
