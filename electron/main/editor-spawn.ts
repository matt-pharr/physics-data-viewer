...
/**
 * The presets surfaced in the Settings dropdown for a given platform.
 *
 * macOS, Linux, and Windows each surface a different subset because most
 * terminals don't exist on the other platforms. `'custom'` and `'none'` are
 * always available as the escape hatches.
 *
 * @param platform - NodeJS platform identifier.
 * @returns Presets to render in the Settings dropdown, in display order.
 */
export function getTerminalPresetsForPlatform(
 ...
 *   contain a literal space without quoting).
 *
 ...
 */
export function tokenizeShellLike(template: string): string[] {
  const tokens: string[] = [];
  let cur = "";
  let inSingle = false;
  let inDouble = false;
...
export function expandTerminalTemplate(
  ...
  const tokens = tokenizeShellLike(template);
  const out: string[] = [];
...
export function resolveEditorSpawn(
  ...
  if (!template) {
    // Either preset='custom' with an empty template, or a preset that doesn't
    // ship a template for this platform (e.g. a config carried across OSes).
    // Fall back to the platform default rather than silently dropping the
    // wrapper for a TUI editor.
    const fallback = defaultTerminalPreset(platform);
    console.warn(
      `[pdv] launchers.terminal.preset='${preset}' has no template for ` +
        `platform '${platform}'; falling back to '${fallback}'.`,
    );
  ...
  return expandTerminalTemplate(template, command, args);
}
